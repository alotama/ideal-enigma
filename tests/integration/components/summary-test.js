...
module('Integration | Component | summary', function (hooks) {
  ...
  test('it should click on open-collapsable button', async function (assert) {
    ...
    assert
      .dom('[data-test="item-count-name"]')
      .hasText('2x Strawberries', 'Found product name');
    assert
      .dom('[data-test="item-price"]')
      .hasText('£40.00', 'Found product price');
  });
});
